...
scanning_data_lock = threading.Lock()
robot_position_lock = threading.Lock()
...
def start_scanning(scanning_duration=10000, step_size=100):
    ...
    def lidar_task(index):
        """Task to listen to LIDAR data for a given index."""
        print(f"Starting LIDAR scan for index {index}...")
        listen_to_lidar(index)  # Assuming this populates frame_scanning_data
        with scanning_data_lock:
            # Ensure thread-safe access (redundant if listen_to_lidar handles it)
            pass
        print(f"Completed LIDAR scan for index {index}.")

    def robot_position_task(index):
        """Task to listen to robot position for a given index."""
        success = node.listen_to_robot_position(index)  # Assuming this populates frame_robot_position_data
        with robot_position_lock:
            # Ensure thread-safe access (redundant if listen_to_robot_position handles it)
            pass
    ...
    for index in range(1, total_steps + 1):
        ...
        # Use ThreadPoolExecutor to run LIDAR and robot position tasks concurrently
        with ThreadPoolExecutor(max_workers=2) as executor:
            # Submit both tasks
            lidar_future = executor.submit(lidar_task, index)
            robot_future = executor.submit(robot_position_task, index)

            # Wait for both tasks to complete
            for future in as_completed([lidar_future, robot_future]):
                if future == robot_future:
                    idx, success = future.result()
                    if success:
                        print(f"Completed robot position capture for index {idx}.")
                    else:
                        print(f"Failed to capture robot position for index {idx}.")

            # Ensure LIDAR task is complete
            lidar_future.result()
    ...
    # Process points (unchanged)
    points = []
    for index in range(1, total_steps + 1):
        with scanning_data_lock, robot_position_lock:
